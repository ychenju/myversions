...
class frame:
    ...
    # def __init__(self, path=None, source=True, parent=None):   # 1. open a wrfout dataset (using path as the parameter), with XLAT, XLONG & XTIME loaded
    #     if source:                                          # or 2. replicate a dataset (when source=False)
    #         self.ncfile = xr.open_dataset(path)
    #         self.lat = np.array(self.ncfile['XLAT']).squeeze()
    #         self.long = np.array(self.ncfile['XLONG']).squeeze()
    #         self.time = np.array(self.ncfile['XTIME']).squeeze()
    #     else:
    #         self.lat = parent.lat
    #         self.long = parent.long
    #         self.time = parent.time
    #         self.__data = parent.getall()
    ...
    def __init__(self, source, *keys):
        ...
        for key in keys:
            self.data[key] = copy.deepcopy(np.array(source[key]).squeeze())
            self.__data[key] = copy.deepcopy(np.array(source[key]).squeeze())


    # def load(self, *keys):    # load any parameter called key (e.g. 'T2')
    #     if self.__source:
    #         for key in keys:
    #             self.__data[key] = np.array(self.ncfile[key]).squeeze()
    #     else:
    #         raise RuntimeError('Duplicated data, not loadable')

    # def loadas(self, key, alias):   # load and rename
    #     if self.__source:
    #         self.__data[alias] = np.array(self.ncfile[key]).squeeze()
    #     else:
    #         raise RuntimeError('Duplicated data, not loadable')
    ...
    def removewater(self):
        if not ('LANDMASK' in self.__data.keys()):
            # self.load('LANDMASK')
            # print("'LANDMASK' is loaded automatically")
